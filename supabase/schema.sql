...
-- =============================================================================
-- 22. VISUAL ASSETS (Image-node visual objects)
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS public.visual_assets (
  ...
  source TEXT NOT NULL CHECK (source IN ('upload', 'drop', 'paste', 'url', 'local-file', 'asset-handle', 'generated', 'backup', 'bridge')),
  source_uri TEXT,
  title TEXT,
  alt_text TEXT,
...
CREATE TABLE IF NOT EXISTS public.visual_asset_versions (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT visual_asset_versions_asset_number_unique UNIQUE (asset_id, version_number)
);
...
ALTER TABLE public.visual_assets
  ...
  FOREIGN KEY (current_version_id) REFERENCES public.visual_asset_versions(id)
...
CREATE TABLE IF NOT EXISTS public.visual_relations (
  ...
  source_type TEXT NOT NULL CHECK (source_type IN ('visual_asset', 'image_node', 'workspace_node', 'step', 'decision', 'doc', 'task', 'habit', 'project', 'focus')),
  source_id TEXT NOT NULL,
  target_type TEXT NOT NULL CHECK (target_type IN ('visual_asset', 'image_node', 'workspace_node', 'step', 'decision', 'doc', 'task', 'habit', 'project', 'focus')),
...
CREATE TRIGGER visual_assets_updated_at BEFORE UPDATE ON public.visual_assets FOR EACH ROW EXECUTE FUNCTION update_updated_at();
DROP TRIGGER IF EXISTS visual_annotations_updated_at ON public.visual_annotations;
CREATE TRIGGER visual_annotations_updated_at BEFORE UPDATE ON public.visual_annotations FOR EACH ROW EXECUTE FUNCTION update_updated_at();
DROP TRIGGER IF EXISTS visual_derived_updated_at ON public.visual_derived;
CREATE TRIGGER visual_derived_updated_at BEFORE UPDATE ON public.visual_derived FOR EACH ROW EXECUTE FUNCTION update_updated_at();
DROP TRIGGER IF EXISTS visual_relations_updated_at ON public.visual_relations;
CREATE TRIGGER visual_relations_updated_at BEFORE UPDATE ON public.visual_relations FOR EACH ROW EXECUTE FUNCTION update_updated_at();
DROP TRIGGER IF EXISTS visual_flow_drafts_updated_at ON public.visual_flow_drafts;
CREATE TRIGGER visual_flow_drafts_updated_at BEFORE UPDATE ON public.visual_flow_drafts FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
CREATE POLICY "Users can insert own visual_assets" ON public.visual_assets FOR INSERT WITH CHECK (auth.uid() = user_id AND (workspace_id IS NULL OR EXISTS (SELECT 1 FROM public.workspaces w WHERE w.id = workspace_id AND w.user_id = auth.uid())));
DROP POLICY IF EXISTS "Users can update own visual_assets" ON public.visual_assets;
CREATE POLICY "Users can update own visual_assets" ON public.visual_assets FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id AND (workspace_id IS NULL OR EXISTS (SELECT 1 FROM public.workspaces w WHERE w.id = workspace_id AND w.user_id = auth.uid())));
...
CREATE POLICY "Users can view own visual_asset_versions" ON public.visual_asset_versions FOR SELECT USING (auth.uid() = user_id AND EXISTS (SELECT 1 FROM public.visual_assets a WHERE a.id = asset_id AND a.user_id = auth.uid()));
DROP POLICY IF EXISTS "Users can insert own visual_asset_versions" ON public.visual_asset_versions;
CREATE POLICY "Users can insert own visual_asset_versions" ON public.visual_asset_versions FOR INSERT WITH CHECK (auth.uid() = user_id AND EXISTS (SELECT 1 FROM public.visual_assets a WHERE a.id = asset_id AND a.user_id = auth.uid()));
DROP POLICY IF EXISTS "Users can delete own visual_asset_versions" ON public.visual_asset_versions;
CREATE POLICY "Users can delete own visual_asset_versions" ON public.visual_asset_versions FOR DELETE USING (auth.uid() = user_id AND EXISTS (SELECT 1 FROM public.visual_assets a WHERE a.id = asset_id AND a.user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can manage own visual_annotations" ON public.visual_annotations;
CREATE POLICY "Users can manage own visual_annotations" ON public.visual_annotations USING (auth.uid() = created_by AND EXISTS (SELECT 1 FROM public.visual_assets a WHERE a.id = asset_id AND a.user_id = auth.uid()) AND EXISTS (SELECT 1 FROM public.workspaces w WHERE w.id = workspace_id AND w.user_id = auth.uid())) WITH CHECK (auth.uid() = created_by AND EXISTS (SELECT 1 FROM public.visual_assets a WHERE a.id = asset_id AND a.user_id = auth.uid()) AND EXISTS (SELECT 1 FROM public.workspaces w WHERE w.id = workspace_id AND w.user_id = auth.uid()));
DROP POLICY IF EXISTS "Users can manage own visual_derived" ON public.visual_derived;
CREATE POLICY "Users can manage own visual_derived" ON public.visual_derived USING (EXISTS (SELECT 1 FROM public.visual_assets a WHERE a.id = asset_id AND a.user_id = auth.uid())) WITH CHECK (EXISTS (SELECT 1 FROM public.visual_assets a WHERE a.id = asset_id AND a.user_id = auth.uid()));
DROP POLICY IF EXISTS "Users can manage own visual_relations" ON public.visual_relations;
CREATE POLICY "Users can manage own visual_relations" ON public.visual_relations USING (auth.uid() = user_id AND EXISTS (SELECT 1 FROM public.workspaces w WHERE w.id = workspace_id AND w.user_id = auth.uid())) WITH CHECK (auth.uid() = user_id AND EXISTS (SELECT 1 FROM public.workspaces w WHERE w.id = workspace_id AND w.user_id = auth.uid()));
DROP POLICY IF EXISTS "Users can manage own visual_flow_drafts" ON public.visual_flow_drafts;
CREATE POLICY "Users can manage own visual_flow_drafts" ON public.visual_flow_drafts USING (auth.uid() = created_by AND EXISTS (SELECT 1 FROM public.visual_assets a WHERE a.id = source_asset_id AND a.user_id = auth.uid()) AND EXISTS (SELECT 1 FROM public.workspaces w WHERE w.id = workspace_id AND w.user_id = auth.uid())) WITH CHECK (auth.uid() = created_by AND EXISTS (SELECT 1 FROM public.visual_assets a WHERE a.id = source_asset_id AND a.user_id = auth.uid()) AND EXISTS (SELECT 1 FROM public.workspaces w WHERE w.id = workspace_id AND w.user_id = auth.uid()));
...
CREATE POLICY "Users can read own visual asset files" ON storage.objects FOR SELECT USING (bucket_id = 'visual-assets' AND auth.uid()::text = split_part(name, '/', 1));
DROP POLICY IF EXISTS "Users can upload own visual asset files" ON storage.objects;
CREATE POLICY "Users can upload own visual asset files" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'visual-assets' AND auth.uid()::text = split_part(name, '/', 1));
DROP POLICY IF EXISTS "Users can delete own visual asset files" ON storage.objects;
CREATE POLICY "Users can delete own visual asset files" ON storage.objects FOR DELETE USING (bucket_id = 'visual-assets' AND auth.uid()::text = split_part(name, '/', 1));

-- PostgREST checks table grants before RLS evaluates the policies above.
GRANT ALL ON TABLE
